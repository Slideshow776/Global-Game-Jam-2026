...
@export var bar_size := Vector2(26, 360)
...
func _ready() -> void:
	...
	green_zone.position = bar_pos + Vector2(0, green_start_px)
	green_zone.size = Vector2(bar_size.x, green_width_px)
	...
	needle.polygon = PackedVector2Array([
		Vector2(0, 0),
		Vector2(needle_width, needle_height * 0.5),
...
func _process(delta: float) -> void:
	...
	needle.position.y += speed * dir * delta

	var top := bar_pos.y
	var bottom := bar_pos.y + bar_size.y - needle_height

	if needle.position.y <= top:
		needle.position.y = top
		dir = 1
	elif needle.position.y >= bottom:
		needle.position.y = bottom
		dir = -1
...
func reset_needle() -> void:
	needle.position = Vector2(
		bar_pos.x + bar_size.x + 4,
		bar_pos.y
	)
	dir = 1

func is_in_green() -> bool:
	var needle_center_y := needle.global_position.y + needle_height * 0.5
	var g0 := green_zone.global_position.y
	var g1 := g0 + green_zone.size.y
	return needle_center_y >= g0 and needle_center_y <= g1
